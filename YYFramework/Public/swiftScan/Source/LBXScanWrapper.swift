...
//    let device = AVCaptureDevice.defaultDevice(withMediaType: AVMediaTypeVideo)
    ...
    /**
     ...
     - returns:
     */
    init(viewStyle: LBXScanViewStyle?, videoPreView:UIView,objType:[AVMetadataObject.ObjectType] = [AVMetadataObject.ObjectType.qr],isCaptureImg:Bool,cropRect:CGRect=CGRect.zero,success:@escaping ( ([LBXScanResult]) -> Void) )
    {
        do{
            if let d = device {
        ...
        guard let put = input else {
            // 照相机没允许
            return
        }
        if session.canAddInput(put)
        {
...
//        output.metadataObjectTypes = [AVMetadataObjectTypeQRCode,AVMetadataObjectTypeEAN13Code, AVMetadataObjectTypeEAN8Code, AVMetadataObjectTypeCode128Code]
        ...
        if let style = viewStyle {
            let x: CGFloat = style.xScanRetangleOffset
            let w: CGFloat = videoPreView.frame.size.width - style.xScanRetangleOffset * 2
            let y: CGFloat = videoPreView.frame.size.height / 2 - w / 2 - style.centerUpOffset
            var frame:CGRect = videoPreView.frame
            frame.origin = CGPoint.zero
            previewLayer?.frame = frame
            // 二维码扫描区域限制
            output.rectOfInterest = CGRect(x: y / frame.size.height, y: x / frame.size.width, width: w / frame.size.height, height: w / frame.size.width)
        } else {
            var frame:CGRect = videoPreView.frame
            frame.origin = CGPoint.zero
            previewLayer?.frame = frame
...
//                print("code string:%@",codeContent)
        ...
        for connection:AnyObject in connections
        {
            ...
            for port:Any in connectionTmp.inputPorts
            {
                if (port as AnyObject).isKind(of: AVCaptureInput.Port.self)
                {
                    ...
                    if AVMediaType.init(portTmp.mediaType.rawValue) == (mediaType)
                    {
    ...
    */
    ...
    {
        let types:[AVMetadataObject.ObjectType] = [.qr, .upce,.code39,.code39Mod43,.ean13,.ean8,.code93, .code128, .pdf417,.aztec]
//        var types:[AVMetadataObject.ObjectType] = [AVMetadataObjectTypeQRCode as NSString, AVMetadataObjectTypeUPCECode as NSString,AVMetadataObjectTypeCode39Code as NSString, AVMetadataObjectTypeCode39Mod43Code as NSString,AVMetadataObjectTypeEAN13Code as NSString, AVMetadataObjectTypeEAN8Code as NSString, AVMetadataObjectTypeCode93Code as NSString,  AVMetadataObjectTypeCode128Code as NSString, AVMetadataObjectTypePDF417Code as NSString, AVMetadataObjectTypeAztecCode as NSString]
