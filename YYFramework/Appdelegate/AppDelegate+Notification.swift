...
extension AppDelegate {
    ...
    func notification_application(_ application: UIApplication, didRegister notificationSettings: UIUserNotificationSettings) {
        
    }
    
    func notification_application(_ application: UIApplication, didReceive notification: UILocalNotification) {
        let state = application.applicationState
        AudioServicesPlaySystemSound(1008)
        if state == .active {
            _playAudio(fileName: notification.soundName ?? "")
    ...
    func notification_application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any]) {
        print(userInfo)
        window?.show("didReceiveRemoteNotification userInfo")
    }
    
    func notification_application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        window?.show("didFailToRegisterForRemoteNotificationsWithError error")
    }
}
...
//            AudioServicesAddSystemSoundCompletion(SystemSoundID, <#T##inRunLoop: CFRunLoop?##CFRunLoop?#>, <#T##inRunLoopMode: CFString?##CFString?#>, <#T##inCompletionRoutine: AudioServicesSystemSoundCompletionProc##AudioServicesSystemSoundCompletionProc##(SystemSoundID, UnsafeMutableRawPointer?) -> Void#>, <#T##inClientData: UnsafeMutableRawPointer?##UnsafeMutableRawPointer?#>)
