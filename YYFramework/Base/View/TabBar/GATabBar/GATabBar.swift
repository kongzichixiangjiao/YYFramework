...
//  YYFramework
//
//  Created by houjianan on 2019/11/23.
...
class GATabBar: UITabBar {
    // 声明代理
    weak var myDelegate: CustomTabBarDelegate?
    
    lazy var plusButton: UIButton = {
        let plusButton = UIButton()
        plusButton.setImage(UIImage.init(named: "tabbar_home_default"), for: .normal)
        plusButton.setImage(UIImage.init(named: "tabbar_home_default"), for: .highlighted)
        plusButton.titleLabel?.font = UIFont.systemFont(ofSize: 11)
        plusButton.setTitle("中间", for: .normal)
        ...
        plusButton.addTarget(self, action: #selector(GATabBar.respondsToPlusButton), for: .touchUpInside)
        return plusButton
    }()
